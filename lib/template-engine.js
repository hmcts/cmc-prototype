...
TemplateEngine.__express = function (templatePath, templateData, next) {
   var templatePaths = _.flatten(_.concat([], [templateData.settings.views, templateData.settings.vendorViews]));
   var templateBase = _.find(templatePaths, (t) => templatePath.indexOf(t) === 0  );
   if (templateBase) {
     var templateName = TemplateEngine._getTemplateName(templateBase, templatePath);
     if (!templateName) {
